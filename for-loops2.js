...
for (var i = start; i < limit; i += increment) {
		...
		newNumber = i;
//"Short"form innerHTML one line (just roll right into .innerHTML)
var printResults = document.getElementById("increment").innerHTML +=`<div>${"the number is " + i}</div>`;
...
var starter = 4096;
var divisor = 2;
for (starter; starter > 1; starter /= divisor) {
	console.log("The Number is now ", starter);

//longform innerHTML two lines (define printNumber, and then add .innerHTML below)
var printNumber = document.getElementById("results");
printNumber.innerHTML += `<div>${"Our new number is " + starter }</div>`;
...
var printPresident = document.getElementById("arrayIteration").innerHTML += `<div>${"The " + (i +1) + " President was " + president[i]}</div>`;
...
var antPrint = document.getElementById("objectIteration").innerHTML += `<div>One of the ants ${" are " + ant}</div>`;
}
